...
// Feature: Yaml Contract Update
// Delegates to: cmd/js-winax-contract/update.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-winax-contract\\update.js";
...
console.log('1️⃣ Yamls contractUpdate Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Yamls contractUpdate Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Yamls contractUpdate failed to spawn:', err.message);
    process.exit(1);
});
