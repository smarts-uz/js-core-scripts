...
// Feature: Word Contract (make .docx from yaml)
// Delegates to: cmd/js-winax-contract/words.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-winax-contract\\words.js";
...
console.log('1️⃣ Word contract Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Word contract Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Word contract failed to spawn:', err.message);
    process.exit(1);
});
