...
// Feature: Olx Merge (phones+online count)
// Delegates to: cmd/js-scraper-olx.uz/merge.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\merge.js";
...
console.log('1️⃣ Olx merge Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx merge Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx merge failed to spawn:', err.message);
    process.exit(1);
});
