...
// Feature: Olx Finder (find phones)
// Delegates to: cmd/js-scraper-olx.uz/finder.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\finder.js";
...
console.log('1️⃣ Olx finder Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx finder Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx finder failed to spawn:', err.message);
    process.exit(1);
});
