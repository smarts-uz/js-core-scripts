...
// Feature: Olx Pages (save pages)
// Delegates to: cmd/js-scraper-olx.uz/pages.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\pages.js";
...
console.log('1️⃣ Olx pages Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx pages Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx pages failed to spawn:', err.message);
    process.exit(1);
});
