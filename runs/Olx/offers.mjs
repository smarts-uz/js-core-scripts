...
// Feature: Olx Offers (save offers)
// Delegates to: cmd/js-scraper-olx.uz/offers.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\offers.js";
...
console.log('1️⃣ Olx offers Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx offers Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx offers failed to spawn:', err.message);
    process.exit(1);
});
