...
// Feature: Olx App Three (phones+merge)
// Delegates to: cmd/js-scraper-olx.uz/App-Three.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\App-Three.js";
...
console.log('1️⃣ Olx appThree Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx appThree Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx appThree failed to spawn:', err.message);
    process.exit(1);
});
