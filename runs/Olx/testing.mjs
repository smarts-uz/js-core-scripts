...
// Feature: Olx Testing (extract+normalize)
// Delegates to: cmd/js-scraper-olx.uz/testing.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\testing.js";
...
console.log('1️⃣ Olx testing Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx testing Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx testing failed to spawn:', err.message);
    process.exit(1);
});
