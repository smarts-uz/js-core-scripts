...
// Feature: Olx Phone (save phones)
// Delegates to: cmd/js-scraper-olx.uz/phone.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\phone.js";
...
console.log('1️⃣ Olx phone Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx phone Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx phone failed to spawn:', err.message);
    process.exit(1);
});
