...
// Feature: Olx App Two (offers+phones)
// Delegates to: cmd/js-scraper-olx.uz/App-Two.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-scraper-olx.uz\\App-Two.js";
...
console.log('1️⃣ Olx appTwo Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Olx appTwo Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Olx appTwo failed to spawn:', err.message);
    process.exit(1);
});
