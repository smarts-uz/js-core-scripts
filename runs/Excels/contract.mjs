...
// Feature: Excel Contract (generate .xlsx from yaml)
// Delegates to: cmd/js-winax-contract/excels.js
...
const SCRIPT = "D:\\Develop\\Projects\\DevApp\\Execute\\JS\\Sources\\cmd\\js-winax-contract\\excels.js";
...
console.log('1️⃣ Excels contract Start →', path.basename(SCRIPT), args.join(' '));
...
child.on('exit', (code) => {
    console.log('3️⃣ Excels contract Done (exit ' + code + ')');
    process.exit(code ?? 0);
});
child.on('error', (err) => {
    console.error('❌ Excels contract failed to spawn:', err.message);
    process.exit(1);
});
