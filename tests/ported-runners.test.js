...
// cmd/ sub-projects (cmd/js-scraper-olx.uz → scripts/Olx/*, cmd/js-winax-contract →
// scripts/Word|Yamls|Excels/contract*). These are NOT reflection-generated, so
// runs-generate.test.js does not cover their content — this file asserts they:
//   1. exist and parse as valid ESM,
//   2. import their classes from '../../classes/…' (call classes directly),
//   3. contain NO reference to a deleted cmd/ path or a spawned child process,
//   4. invoke the exact class methods each ported feature is defined by.
...
const RUNS = path.join(ROOT, 'scripts');
...
describe('ported runners are self-contained (no cmd spawn)', () => {
  it.each(Object.keys(ALL))(
    '%s imports classes from ../../classes and never references cmd/',
    (rel) => {
      const src = read(rel);
      // imports at least one class from the sibling classes/ tree
      expect(src).toMatch(/await import\('\.\.\/\.\.\/classes\/\w+\.js'\)/);
...
describe('contract runners call their exact Word/Yamls/Excels method', () => {
  ...
  it.each([
    'Word/contract.mjs',
    'Yamls/contractFill.mjs',
    'Yamls/contractUpdate.mjs',
    'Excels/contract.mjs',
  ])('%s supports single --yaml and batch --all', (rel) => {
    const src = read(rel);
