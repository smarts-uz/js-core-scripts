  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      ...
      if (isExcludedDir(entry.name)) {
        console.info(`⏭️  excluded @-folder: ${path.relative(ROOT, path.join(dir, entry.name))}`);
      }
      continue;
    }
...
async function main() {
  console.info(`[ALL] 🟢 scanning ${path.relative(ROOT, HERE) || 'tests'} (top level only) for *.test.js (excluding @-folders)…`);
  ...
  console.info(`[ALL] filters = ${JSON.stringify(filters)}, jest flags = ${JSON.stringify(passThrough)}`);

  let testFiles = collectTests(HERE).map((p) => path.relative(ROOT, p).replaceAll('\\', '/'));
  if (filters.length) {
    testFiles = testFiles.filter((f) => filters.some((flt) => f.toLowerCase().includes(flt.toLowerCase())));
  }
  ...
  if (!existsSync(jestBin)) {
    console.error(`[ALL] ❌ jest binary not found at ${jestBin}. Run from a project with node_modules.`);
    process.exit(1);
  }
  ...
  const jestArgs = [
    '--rootDir', ROOT,
    '--testPathIgnorePatterns', '/node_modules/', '--testPathIgnorePatterns', '/@[^/]*/',
    ...passThrough,
  ...
  child.on('exit', (code) => {
    console.info(`[ALL] ${code === 0 ? '✅ all suites passed' : `❌ jest exited with code ${code}`}`);
    process.exit(code ?? 1);
  });
}
