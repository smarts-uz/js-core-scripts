...
### `Accrual`/`Payment`/`Loaners`/`Penalty` — the code-driven chain (see the fuller rental-reconciliation rule block below for the calculation rules; this section documents the concrete implementing code)

`Yamls.fillYamlWithInfo` writes all four blocks on every contract fill via `Yamls.recomputeChain` (`classes/Yamls.js`) — **NOT** `computePunish`/`writePunish`, which no longer exist in this codebase (an earlier draft of this documentation described that shape; it was superseded by the chain below and never matched the shipped code):

- **`Yamls.recomputeChain(startDate, futureDate, price, priceMax, payments)`** — builds `Accrual` at `price` via `buildAccrualEntries`, chains real `payments` across it via `computePaymentChain` to get `Payment`/`Loaners`, then iterates `applyPriceMaxToDebtMonths` + `computePaymentChain` to a fixed point (a month with `Loaners > 0` gets re-priced at `priceMax`, which can change which months are in debt, so the whole chain re-runs until `Loaners` stops changing). Appends a trailing `{ ALL: sum }` entry to each of the three returned arrays.
- **`Yamls.computePenalty(accrual, loaners, capRatio)`** — for every month with `Loaners > 0`, penalty is `capRatio` (default `0.5`, `config.yml`'s `Penalty.CapRatio`) of that month's own `Accrual` figure, applied immediately (no daily accrual, no due-date gating) — capped a second time by the running `Accrual` total so far. Appends its own trailing `{ ALL: sum }`.
- **`Yamls.actualPayments(yamlData)`** — real incoming cash: `Bank-OT` + `Trans-OT` + `Card-OT` only. `EHF-IN` (an invoice, not cash), `Bank-IN`/`Card-IN` (refunds), `Bonuses`/`BaaR-OT`/`BaaR-IN` are excluded.
- **`Yamls.writeAccrual`/`writePayment`/`writeLoaners`/`writePenalty`** — each a thin `writeYamlArraySection` wrapper, chained via `afterKey` so the on-disk block order is always `Accrual` → `Payment` → `Loaners` → `Penalty` → `Bank-OT` → `Trans-OT` → `Card-OT` → … (`Bank-OT`/`Trans-OT`/`Card-OT` grouped adjacent per the field-ordering rule below).
...
- **`Excels.processAccrual`/`processPayment`/`processLoaners`/`processPenalty`** (`classes/Excels.js`) mirror each `.yml` block into the Excel report via their own `{Accrual}`/`{Payment}`/`{Loaners}`/`{Penalty}` placeholder cells — `processPenalty` alone is gated on `yamlData.PenaltyON === true` (blanking its own placeholder when not true, since the generic `{KEY}`-clear pass in `generate()` skips array-valued keys). All four warn-and-skip (never crash) when their placeholder is missing from the template. `templa\ALL.contract`'s toggle field is `PenaltyON` (renamed from an earlier `PunishEnable`), bundled `false` by default, owned/documented by `smarts-firm-docums`.
- Regression tests: `tests/Dates.test.js` → `Dates.daysBetween`/`Dates.daysInMonth`; `tests/Yamls.test.js` → `Yamls.actualPayments` + `Yamls.recomputeChain` + `Yamls.computePenalty` + `Yamls.writeAccrual`/`writePayment`/`writeLoaners`/`writePenalty` + `Yamls.scanCellFolder`'s format-variant/dedup cases; `tests/Excels.test.js` → `Excels.processAccrual`/`processPayment`/`processLoaners`/`processPenalty` (PenaltyON gate, missing-placeholder no-op case). **These test files were not updated in this session — verify they match the current `recomputeChain`/`computePenalty` shape before trusting them; an update pass is still owed.**
...
- **A partial-month period's accrual is prorated by real day count, then ROUNDED UP to the nearest 1,000 UZS — never left as a raw fractional-so'm figure.** Formula: `ceil((daysInPeriod / daysInThatCalendarMonth) * monthlyRate / 1000) * 1000`. E.g. a period from day 9 to the end of a 31-day month is 23 days: `23/31 × 390,000 = 289,354.84` → rounded up to **290,000** (never left as `289,354.84`, and never plain-rounded down/nearest — always ceiling to the next 1,000). Every other `Accrual`/`Punish` entry in a tenant's `.yml` file already uses whole-thousand figures; a partial-period entry must match that same whole-thousand shape.
- **A tenant `.yml` file's `Accrual`/`Payment`/`Loaners`/`Penalty` blocks share the SAME period list, in the SAME order (each a `start#end: amount` array under `StartDate`…`FutureDate`), plus a trailing `- ALL: <sum>` entry summing that block's own period values.** `Payment` is the mirror of `Loaners` — for each period, `Payment[i] + Loaners[i] = Accrual[i]` always holds. `Payment[i]` is derived from the SAME chained-coverage allocation as `Loaners` (per the payment-chain rule above, across `Bank-OT` + `Trans-OT` + `Card-OT` only — `EHF-IN` is an issued invoice/document, never counted as real received cash unless the tenant folder's own evidence proves otherwise for that specific tenant).
- **The per-period 50%-of-that-month penalty cap (§21.3's per-period reading) is DISTINCT from, and does not replace, an OVERALL cap across the whole rental term** — the same clause also caps total accrued penalty at 50% of total rent owed over the full contract term. When computing `Penalty`, check both: no single period's penalty exceeds 50% of that period's own `Accrual`, AND the running sum of `Penalty` never exceeds 50% of the running sum of `Accrual` (`Accrual`'s own `ALL` figure) — if the second cap would bind, it is the tighter constraint and wins.
- **Field ordering in a tenant `.yml`:** `Bank-OT`, `Trans-OT`, `Card-OT` (the three real-cash-payment sources feeding the `Payment`/`Loaners` chain) are grouped adjacent to each other, in that order; the `*-IN` (return/refund) counterparts and `EHF-IN`/`Bonuses`/`BaaR-*` keys follow after. The penalty block's YAML key is named `Penalty` (not `Punish`); its enable/disable toggle flag is `PenaltyON` (renamed from the earlier `PunishEnable`). Exactly one blank line separates each top-level array block (`Accrual`/`Payment`/`Loaners`/`Penalty`/`Bank-OT`/…) from the next — never between the individual `- key: value` items inside one block.
- **A month with `Loaners > 0` (i.e. underpaid or unpaid) is re-accrued at that tariff's `PriceMax` (from `conf/cost/<Tariff>.yaml`) instead of the normal `Price`** — `Price` is the rate that applies only when paid on time; missing/short payment for a period switches that period's own `Accrual` entry to `PriceMax` (prorated + rounded up the same way as any partial period, per the rounding rule above). Only months currently showing `Loaners > 0` get this treatment — a month that is fully paid keeps `Price`, even if an earlier or later month is in debt. Recompute the whole chain after this substitution: a changed `Accrual` entry changes that block's own `ALL` sum, which in turn changes `Payment`/`Loaners`/`Penalty` for every period from that point on (the chain is re-run end to end, not patched in place).
- **`Penalty` for a given period is 50% of that period's (possibly `PriceMax`-adjusted) `Accrual` figure whenever `Loaners > 0` for that period — applied immediately, not gated on the due date having passed.** This is a stricter reading than "wait for the due date, then accrue daily up to the cap" — the moment a period shows any outstanding `Loaners`, its `Penalty` entry is set straight to the 50%-of-`Accrual` cap for that period, full stop, no partial daily buildup tracked separately.
- **The whole `Accrual`→`Payment`→`Loaners`→`Penalty` chain is code-driven, never hand-edited as the source of truth.** `Yamls.recomputeChain` (which calls `buildAccrualEntries` → `applyPriceMaxToDebtMonths` → `computePaymentChain`, iterated to a fixed point) and `Yamls.computePenalty` are the single authoritative implementation — `Yamls.fillYamlWithInfo` calls them and writes the four blocks via `writeAccrual`/`writePayment`/`writeLoaners`/`writePenalty`. A tenant `.yml` may be hand-edited for a quick manual check, but any real recompute (a new payment folder appears, `PriceMax` changes, the period window shifts) goes through re-running `fillYamlWithInfo`, not manual arithmetic in the file.
- **`Excel.CellNames` in `config.yml` is the Excel _template's own column layout_ (`Trans-OT`, `Bank-OT`, `Bank-IN`, `EHF-IN`, `Card-OT`, `Card-IN`, `Bonuses`, `BaaR-OT`, `BaaR-IN`) and is NOT reordered to match a tenant `.yml`'s field order** — the two orderings serve different purposes (Excel physical column position vs. `.yml` readability) and are allowed to differ.
- **`Excels.generate` mirrors the `.yml` chain into the Excel report via four placeholder-driven writers, each reading straight from `yamlData` (never re-deriving/recomputing in Excels.js itself):** `processAccrual` (`{Accrual}` placeholder, from `yamlData.Accrual`), `processPayment` (`{Payment}`, from `yamlData.Payment`, unconditional), `processLoaners` (`{Loaners}`, from `yamlData.Loaners`, unconditional), `processPenalty` (`{Penalty}`, from `yamlData.Penalty`, gated on `yamlData.PenaltyON === true` — mirrors the old `PunishEnable` gate under its new name). Each skips the trailing `{ ALL: sum }` entry (no date to write a row for) and warns-and-skips (never crashes) when its own placeholder cell is missing from the template — the Excel template can lag the `.yml` schema without breaking `generate`.
- **The Excel TEMPLATE file (`Templates.Excel` in `config.yml`, now `d:\Humans\Building\Rentalls\ActReco\Projects\Act 97.xlsx`) carries its own `{Accrual}`/`{Payment}`/`{Loaners}`/`{Penalty}` placeholder cells, so `processAccrual`/`processPayment`/`processLoaners`/`processPenalty` write into a real target — verified with a real `Excels.generate` run against ASHALIFE PHARMA's contract, confirming every one of these placeholders resolves.** The old `{Pricings}` placeholder name was retired in favor of `{Accrual}` (matching the `.yml` key everywhere); `{StartDateExcel}`/`{ComDateExcel}`/`{FutureDateAppExcel}` were likewise retired in favor of `{StartDate}`/`{ComDate}`/`{FutureDateApp}` (the plain `.yml` key names, no `Excel` suffix). The stale per-row DATEDIF/LET proration formulas that pre-dated the `.yml`-driven chain (in the original template's `E`/`F` columns) were removed (cleared to static `0`, not just the two cells that originally seeded Excel's Shared Formula group — clearing only those re-anchored the formula onto neighboring cells instead of removing it, a real trap hit and fixed this session); the one genuinely-needed formula, the Accrual grand-total `SUM`, was kept and is now fed by `processAccrual` also writing into the sheet's "Сумма" total column. **Each of the four `start#end`-format blocks (`Accrual`/`Payment`/`Loaners`/`Penalty` — never the 9 single-date blocks like `Bank-OT`/`Card-OT`) now has a real two-column "Начало"/"Конец" pair (the old single "Дата" header renamed to "Начало", a new "Конец" column inserted right after it via Excel's own Insert-Column, processed right-to-left across the sheet so an earlier insert never shifts a later block's column index) — `processAccrual`/`processPayment`/`processLoaners`/`processPenalty` all split each `.yml` interval key on `#` and write both dates.** Each Excel template revision is a real increment-versioned copy (`Act 91.xlsx` → `92` → … → `97`, every prior version left untouched) produced via `smarts-winax-excel`, never a hand-rolled `winax` call. **Any future edit to this Excel template (a new column, a style change, a placeholder rename) goes through `smarts-winax-excel`'s `edit`/other revise-verb action — never a direct `winax.Object('Excel.Application')` call in a session script** — the skill owns increment-versioning (`Act N+1.xlsx` beside the source), safe-open/repair, and template-style reuse; bypassing it was a real mistake caught and corrected in this project's history.
- **`Excels.processPenalty` must blank its own `{Penalty}` placeholder when `yamlData.PenaltyON` is not `true` — never rely on `generate()`'s generic `{KEY}`-placeholder-clear pass for this.** That pass only replaces SCALAR `yamlData` values and explicitly skips every array-valued key (`if (Array.isArray(value)) continue;`); since `Penalty` is always an array, the generic pass never reaches `{Penalty}`, so a real `Excels.generate` run left the literal `"{Penalty}"` text sitting in the report until `processPenalty` was fixed to call `this.replaceInSheet('{Penalty}', '')` itself in its `PenaltyON !== true` branch.
