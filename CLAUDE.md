...
- **The Excel TEMPLATE file itself (`Templates.Excel` in `config.yml`, currently `d:\Humans\Building\Rentalls\ActReco\Projects\Act 91.xlsx`) still needs its own `{Accrual}`/`{Payment}`/`{Loaners}`/`{Penalty}` placeholder cells added via real Excel/COM edits before `processPayment`/`processLoaners`/`processPenalty` can actually write anything — this is a separate, not-yet-done task from the code above.** The old `{Pricings}` placeholder name is retired in favor of `{Accrual}` (matching the `.yml` key everywhere) — when adding the new cells, rename any existing `{Pricings}` cell to `{Accrual}` too, rather than leaving both.
