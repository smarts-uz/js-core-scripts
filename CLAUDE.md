...
- **`node_modules/`, `package.json`, and `package-lock.json` NO LONGER live in this project root — they were moved to `C:\Users\Administrator\.ai\node\` (a machine-wide npm/node store `smarts-install-npm` also installs into).** This project root currently has **no local `node_modules` and no working module-resolution mechanism** — this is a deliberate, known, temporary state, not an oversight. Because this project is pure ESM (`"type": "module"`), `NODE_PATH` **cannot** fix this even if set: verified live by launching `node` with `NODE_PATH=C:\Users\Administrator\.ai\node\node_modules` set beforehand and running `import('yargs')` from this project's directory — it still failed with `Cannot find package 'yargs'`. `NODE_PATH` only affects CommonJS `require()` resolution, never the ESM `import` resolution algorithm. Every `scripts/<Class>/<method>.mjs` runner will fail with `MODULE_NOT_FOUND`/`ERR_MODULE_NOT_FOUND` until a real fix is put in place (the working option, confirmed but not yet applied: a genuine Windows directory symlink `node_modules → C:\Users\Administrator\.ai\node\node_modules` created via `cmd`'s `mklink /D` — **not** Git Bash's `ln -s`, which was tested live here and silently fell back to a full recursive **copy** instead of a real link when linking across drives, confirmed by comparing `stat` inode/device numbers on both sides).
- **`package.json`'s real content (runtime deps + dev tooling) now lives ONLY at `C:\Users\Administrator\.ai\node\package.json`.** It still declares `"type": "module"`, `engines.node >= 20`, the jest scripts, the quality scripts, the `generate:*` scripts, `dependencies` (every runtime package `classes/`/`scripts/` imports — `winax`, `yargs`, `puppeteer`, `js-yaml`, `turndown`, `xlsx`, `pino`, `dotenv`, etc. — plus the vendored global CLI tools, see below), and `devDependencies`. Before trusting it, cross-check `C:\Users\Administrator\.ai\node\node_modules\<pkg>` actually exists for anything `classes/`/`scripts/` imports — a whole-file rewrite has previously dropped the entire real `dependencies` block by accident (see the CLI-tools note below).
...
- [classes/](classes/) is the runtime class library, a real local folder in this root — imported by **relative path** (`./classes/Yamls.js`, `./classes/Word.js`, etc.). `node_modules/` itself is **not** local anymore — see the `node_modules` note above (moved to `C:\Users\Administrator\.ai\node\`, currently no working link back).
