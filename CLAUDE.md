...
- **`node_modules/` at this project root is a real Windows directory symlink → `C:\Users\Administrator\.ai\node\node_modules` (the machine-wide npm/node store `smarts-install-npm` also installs into) — confirmed live and working.** Because this project is pure ESM (`"type": "module"`), a `NODE_PATH` env var alone cannot fix module resolution here (`NODE_PATH` only affects CommonJS `require()`, never the ESM `import` resolution algorithm) — the real symlink is what makes every `scripts/<Class>/<method>.mjs` runner resolve its imports correctly. Re-verified 2026-07-27: `fs.lstatSync('node_modules').isSymbolicLink()` → `true`, and the full jest suite (907 tests) ran successfully straight through it via `node node_modules/jest/bin/jest.js` (the `node_modules/.bin/*.cmd` wrappers themselves don't run cleanly from Git Bash — invoke the real `.js` bin under `node_modules/<pkg>/bin/` directly instead of the `.cmd` shim when running tools from Bash). If this symlink is ever found missing/broken again, recreate it via `cmd`'s `mklink /D` — **not** Git Bash's `ln -s`, which silently falls back to a full recursive **copy** instead of a real link when linking across drives (confirmed by comparing `stat` inode/device numbers on both sides in an earlier investigation).
