// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Homoglyph.markdownAsk.
// Edit the method in humanize/classes/Homoglyph.js, then re-run the generator to refresh.
// Calls Homoglyph.markdownAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Homoglyph markdownAsk Start');
    ...
    const run = async () => {
        const result = await Homoglyph.markdownAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Homoglyph markdownAsk Error');
    ...
    console.log('3️⃣ Homoglyph markdownAsk Done');
}
