// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Homoglyph.word.
// Edit the method in humanize/classes/Homoglyph.js, then re-run the generator to refresh.
// Calls Homoglyph.word(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Homoglyph word Start');
    ...
    const run = async () => {
        const result = await Homoglyph.word(argv["file"], argv["chars"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Homoglyph word Error');
    ...
    console.log('3️⃣ Homoglyph word Done');
}
