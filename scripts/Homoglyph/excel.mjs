// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Homoglyph.excel.
// Edit the method in classes/Homoglyph.js, then re-run the generator to refresh.
// Calls Homoglyph.excel(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Homoglyph excel Start');
    ...
    const run = async () => {
        const result = await Homoglyph.excel(argv["file"], argv["chars"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Homoglyph excel Error');
    ...
    console.log('3️⃣ Homoglyph excel Done');
}
