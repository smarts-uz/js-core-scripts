// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Homoglyph.wordAsk.
// Edit the method in humanize/classes/Homoglyph.js, then re-run the generator to refresh.
// Calls Homoglyph.wordAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Homoglyph wordAsk Start');
    ...
    const run = async () => {
        const result = await Homoglyph.wordAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Homoglyph wordAsk Error');
    ...
    console.log('3️⃣ Homoglyph wordAsk Done');
}
