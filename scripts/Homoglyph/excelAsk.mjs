// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Homoglyph.excelAsk.
// Edit the method in humanize/classes/Homoglyph.js, then re-run the generator to refresh.
// Calls Homoglyph.excelAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Homoglyph excelAsk Start');
    ...
    const run = async () => {
        const result = await Homoglyph.excelAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Homoglyph excelAsk Error');
    ...
    console.log('3️⃣ Homoglyph excelAsk Done');
}
