// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.scrapePages.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.scrapePages(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe scrapePages Start');
    ...
    const run = async () => {
        const result = await Puppe.scrapePages(argv["url"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe scrapePages Error');
    ...
    console.log('3️⃣ Puppe scrapePages Done');
}
