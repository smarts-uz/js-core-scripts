// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.extractOffers.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.extractOffers(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe extractOffers Start');
    ...
    const run = async () => {
        const result = await Puppe.extractOffers();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe extractOffers Error');
    ...
    console.log('3️⃣ Puppe extractOffers Done');
}
