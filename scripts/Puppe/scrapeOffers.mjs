// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.scrapeOffers.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.scrapeOffers(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe scrapeOffers Start');
    ...
    const run = async () => {
        const result = await Puppe.scrapeOffers(argv["url"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe scrapeOffers Error');
    ...
    console.log('3️⃣ Puppe scrapeOffers Done');
}
