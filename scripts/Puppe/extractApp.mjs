// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.extractApp.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.extractApp(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe extractApp Start');
    ...
    const run = async () => {
        const result = await Puppe.extractApp(argv["pattern"], argv["page"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe extractApp Error');
    ...
    console.log('3️⃣ Puppe extractApp Done');
}
