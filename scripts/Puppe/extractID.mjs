// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.extractID.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.extractID(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe extractID Start');
    ...
    const run = async () => {
        const result = await Puppe.extractID(argv["page"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe extractID Error');
    ...
    console.log('3️⃣ Puppe extractID Done');
}
