// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.saveAsMhtml.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.saveAsMhtml(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe saveAsMhtml Start');
    ...
    const run = async () => {
        const result = await Puppe.saveAsMhtml(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe saveAsMhtml Error');
    ...
    console.log('3️⃣ Puppe saveAsMhtml Done');
}
