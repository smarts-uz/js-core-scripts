// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.itemSavePagination.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.itemSavePagination(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe itemSavePagination Start');
    ...
    const run = async () => {
        const result = await Puppe.itemSavePagination(argv["url"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe itemSavePagination Error');
    ...
    console.log('3️⃣ Puppe itemSavePagination Done');
}
