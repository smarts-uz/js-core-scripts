// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.extractUserId.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.extractUserId(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe extractUserId Start');
    ...
    const run = async () => {
        const result = await Puppe.extractUserId();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe extractUserId Error');
    ...
    console.log('3️⃣ Puppe extractUserId Done');
}
