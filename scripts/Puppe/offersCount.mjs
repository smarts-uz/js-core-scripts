// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.offersCount.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.offersCount(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe offersCount Start');

    const argv = yargs(hideBin(process.argv))
        .option("fullPath", { describe: "fullPath" })
        .help()
        .parse();

    const run = async () => {
        const result = await Puppe.offersCount(argv["fullPath"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe offersCount Error');
    ...
    console.log('3️⃣ Puppe offersCount Done');
}
