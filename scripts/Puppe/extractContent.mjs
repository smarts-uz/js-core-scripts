// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.extractContent.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.extractContent(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe extractContent Start');
    ...
    const run = async () => {
        const result = await Puppe.extractContent(argv["page"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe extractContent Error');
    ...
    console.log('3️⃣ Puppe extractContent Done');
}
