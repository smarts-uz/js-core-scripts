// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Puppe.scrapePhone.
// Edit the method in classes/Puppe.js, then re-run the generator to refresh.
// Calls Puppe.scrapePhone(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Puppe scrapePhone Start');
    ...
    const run = async () => {
        const result = await Puppe.scrapePhone(argv["url"], argv["userIdPath"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Puppe scrapePhone Error');
    ...
    console.log('3️⃣ Puppe scrapePhone Done');
}
