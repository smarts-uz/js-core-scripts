// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for ES.execute.
// Edit the method in classes/ES.js, then re-run the generator to refresh.
// Calls ES.execute(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ ES execute Start');

    const argv = yargs(hideBin(process.argv))
        .option("command", { describe: "command" })
        .help()
        .parse();

    const run = async () => {
        const result = await ES.execute(argv["command"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'ES execute Error');
    ...
    console.log('3️⃣ ES execute Done');
}
