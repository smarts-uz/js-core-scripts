// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Secrets.env.
// Edit the method in classes/Secrets.js, then re-run the generator to refresh.
// Calls Secrets.env(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Secrets env Start');
    ...
    const run = async () => {
        const result = await Secrets.env(argv["name"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Secrets env Error');
    ...
    console.log('3️⃣ Secrets env Done');
}
