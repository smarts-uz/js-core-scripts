// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.initFolders.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.initFolders(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word initFolders Start');
    ...
    const run = async () => {
        const result = await Word.initFolders(argv["yaml"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word initFolders Error');
    ...
    console.log('3️⃣ Word initFolders Done');
}
