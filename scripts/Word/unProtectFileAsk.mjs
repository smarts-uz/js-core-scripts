// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.unProtectFileAsk.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.unProtectFileAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word unProtectFileAsk Start');
    ...
    const run = async () => {
        const result = await Word.unProtectFileAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word unProtectFileAsk Error');
    ...
    console.log('3️⃣ Word unProtectFileAsk Done');
}
