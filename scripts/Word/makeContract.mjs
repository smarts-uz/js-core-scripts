// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.makeContract.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.makeContract(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word makeContract Start');
    ...
    const run = async () => {
        const result = await Word.makeContract(argv["yaml"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word makeContract Error');
    ...
    console.log('3️⃣ Word makeContract Done');
}
