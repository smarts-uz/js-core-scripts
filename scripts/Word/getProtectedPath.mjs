// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.getProtectedPath.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.getProtectedPath(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word getProtectedPath Start');
    ...
    const run = async () => {
        const result = await Word.getProtectedPath(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word getProtectedPath Error');
    ...
    console.log('3️⃣ Word getProtectedPath Done');
}
