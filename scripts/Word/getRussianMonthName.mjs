// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.getRussianMonthName.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.getRussianMonthName(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word getRussianMonthName Start');

    const argv = yargs(hideBin(process.argv))
        .option("monthNumber", { describe: "monthNumber" })
        .help()
        .parse();

    const run = async () => {
        const result = await Word.getRussianMonthName(argv["monthNumber"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word getRussianMonthName Error');
    ...
    console.log('3️⃣ Word getRussianMonthName Done');
}
