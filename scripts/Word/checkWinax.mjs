// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.checkWinax.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.checkWinax(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word checkWinax Start');
    ...
    const run = async () => {
        const result = await Word.checkWinax(argv["methodName"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word checkWinax Error');
    ...
    console.log('3️⃣ Word checkWinax Done');
}
