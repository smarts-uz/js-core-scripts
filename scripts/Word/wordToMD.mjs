// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.wordToMD.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.wordToMD(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word wordToMD Start');
    ...
    const run = async () => {
        const result = await Word.wordToMD(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word wordToMD Error');
    ...
    console.log('3️⃣ Word wordToMD Done');
}
