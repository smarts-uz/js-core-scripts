// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.getNumberWordOnly.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.getNumberWordOnly(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word getNumberWordOnly Start');

    const argv = yargs(hideBin(process.argv))
        .option("num", { describe: "num" })
        .help()
        .parse();

    const run = async () => {
        const result = await Word.getNumberWordOnly(argv["num"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word getNumberWordOnly Error');
    ...
    console.log('3️⃣ Word getNumberWordOnly Done');
}
