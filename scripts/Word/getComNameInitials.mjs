// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Word.getComNameInitials.
// Edit the method in classes/Word.js, then re-run the generator to refresh.
// Calls Word.getComNameInitials(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Word getComNameInitials Start');
    ...
    const run = async () => {
        const result = await Word.getComNameInitials(argv["name"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Word getComNameInitials Error');
    ...
    console.log('3️⃣ Word getComNameInitials Done');
}
