// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Category.loadYaml.
// Edit the method in classes/Category.js, then re-run the generator to refresh.
// Calls Category.loadYaml(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Category loadYaml Start');
    ...
    const run = async () => {
        const result = await Category.loadYaml(argv["file"], argv["tag"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Category loadYaml Error');
    ...
    console.log('3️⃣ Category loadYaml Done');
}
