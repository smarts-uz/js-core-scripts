// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Category.ensureDir.
// Edit the method in classes/Category.js, then re-run the generator to refresh.
// Calls Category.ensureDir(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Category ensureDir Start');
    ...
    const run = async () => {
        const result = await Category.ensureDir(argv["dirPath"], argv["tag"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Category ensureDir Error');
    ...
    console.log('3️⃣ Category ensureDir Done');
}
