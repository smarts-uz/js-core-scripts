// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Category.revert.
// Edit the method in classes/Category.js, then re-run the generator to refresh.
// Calls Category.revert(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Category revert Start');

    const argv = yargs(hideBin(process.argv))
        .option("yaml", { describe: "yamlPath" })
        .help()
        .parse();

    const run = async () => {
        const result = await Category.revert(argv["yaml"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Category revert Error');
    ...
    console.log('3️⃣ Category revert Done');
}
