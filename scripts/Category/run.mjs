// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Category.run.
// Edit the method in classes/Category.js, then re-run the generator to refresh.
// Calls Category.run(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Category run Start');
    ...
    const run = async () => {
        const result = await Category.run(argv["yaml"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Category run Error');
    ...
    console.log('3️⃣ Category run Done');
}
