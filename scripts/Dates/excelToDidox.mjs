// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.excelToDidox.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.excelToDidox(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates excelToDidox Start');
    ...
    const run = async () => {
        const result = await Dates.excelToDidox(argv["date"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates excelToDidox Error');
    ...
    console.log('3️⃣ Dates excelToDidox Done');
}
