// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.monthEnd.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.monthEnd(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates monthEnd Start');
    ...
    const run = async () => {
        const result = await Dates.monthEnd(argv["dateExcel"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates monthEnd Error');
    ...
    console.log('3️⃣ Dates monthEnd Done');
}
