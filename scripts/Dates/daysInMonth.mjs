// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.daysInMonth.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.daysInMonth(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates daysInMonth Start');

    const argv = yargs(hideBin(process.argv))
        .option("dateExcel", { describe: "dateExcel" })
        .help()
        .parse();

    const run = async () => {
        const result = await Dates.daysInMonth(argv["dateExcel"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates daysInMonth Error');
    ...
    console.log('3️⃣ Dates daysInMonth Done');
}
