// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.parseDMY.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.parseDMY(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates parseDMY Start');
    ...
    const run = async () => {
        const result = await Dates.parseDMY(argv["dateStr"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates parseDMY Error');
    ...
    console.log('3️⃣ Dates parseDMY Done');
}
