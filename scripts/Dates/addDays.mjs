// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.addDays.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.addDays(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates addDays Start');

    const argv = yargs(hideBin(process.argv))
        .option("dateStr", { describe: "dateStr" })
        .option("days", { describe: "days" })
        .help()
        .parse();

    const run = async () => {
        const result = await Dates.addDays(argv["dateStr"], argv["days"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates addDays Error');
    ...
    console.log('3️⃣ Dates addDays Done');
}
