// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.randomIntOne.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.randomIntOne(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates randomIntOne Start');
    ...
    const run = async () => {
        const result = await Dates.randomIntOne(argv["value"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates randomIntOne Error');
    ...
    console.log('3️⃣ Dates randomIntOne Done');
}
