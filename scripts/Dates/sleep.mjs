// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.sleep.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.sleep(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates sleep Start');

    const argv = yargs(hideBin(process.argv))
        .option("ms", { describe: "ms" })
        .option("random", { describe: "random (optional)" })
        .help()
        .parse();

    const run = async () => {
        const result = await Dates.sleep(argv["ms"], argv["random"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates sleep Error');
    ...
    console.log('3️⃣ Dates sleep Done');
}
