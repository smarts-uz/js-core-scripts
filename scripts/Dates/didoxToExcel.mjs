// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.didoxToExcel.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.didoxToExcel(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates didoxToExcel Start');
    ...
    const run = async () => {
        const result = await Dates.didoxToExcel(argv["date"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates didoxToExcel Error');
    ...
    console.log('3️⃣ Dates didoxToExcel Done');
}
