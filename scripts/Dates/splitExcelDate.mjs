// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.splitExcelDate.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.splitExcelDate(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates splitExcelDate Start');
    ...
    const run = async () => {
        const result = await Dates.splitExcelDate(argv["date"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates splitExcelDate Error');
    ...
    console.log('3️⃣ Dates splitExcelDate Done');
}
