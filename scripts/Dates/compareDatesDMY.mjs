// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.compareDatesDMY.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.compareDatesDMY(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates compareDatesDMY Start');
    ...
    const run = async () => {
        const result = await Dates.compareDatesDMY(argv["a"], argv["b"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates compareDatesDMY Error');
    ...
    console.log('3️⃣ Dates compareDatesDMY Done');
}
