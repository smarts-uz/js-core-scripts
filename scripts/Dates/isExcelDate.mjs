// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.isExcelDate.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.isExcelDate(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates isExcelDate Start');
    ...
    const run = async () => {
        const result = await Dates.isExcelDate(argv["date"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates isExcelDate Error');
    ...
    console.log('3️⃣ Dates isExcelDate Done');
}
