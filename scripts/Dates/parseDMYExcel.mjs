// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.parseDMYExcel.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.parseDMYExcel(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates parseDMYExcel Start');
    ...
    const run = async () => {
        const result = await Dates.parseDMYExcel(argv["dateStr"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates parseDMYExcel Error');
    ...
    console.log('3️⃣ Dates parseDMYExcel Done');
}
