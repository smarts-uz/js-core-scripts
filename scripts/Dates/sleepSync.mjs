// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.sleepSync.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.sleepSync(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates sleepSync Start');
    ...
    const run = async () => {
        const result = await Dates.sleepSync(argv["ms"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates sleepSync Error');
    ...
    console.log('3️⃣ Dates sleepSync Done');
}
