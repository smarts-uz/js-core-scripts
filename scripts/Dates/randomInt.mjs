// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Dates.randomInt.
// Edit the method in classes/Dates.js, then re-run the generator to refresh.
// Calls Dates.randomInt(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Dates randomInt Start');
    ...
    const run = async () => {
        const result = await Dates.randomInt(argv["min"], argv["max"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Dates randomInt Error');
    ...
    console.log('3️⃣ Dates randomInt Done');
}
