// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.saveRegions.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.saveRegions(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox saveRegions Start');
    ...
    const run = async () => {
        const result = await Didox.saveRegions();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox saveRegions Error');
    ...
    console.log('3️⃣ Didox saveRegions Done');
}
