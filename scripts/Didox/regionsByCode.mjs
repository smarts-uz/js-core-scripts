// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.regionsByCode.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.regionsByCode(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox regionsByCode Start');

    const argv = yargs(hideBin(process.argv))
        .option("code", { describe: "code" })
        .help()
        .parse();

    const run = async () => {
        const result = await Didox.regionsByCode(argv["code"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox regionsByCode Error');
    ...
    console.log('3️⃣ Didox regionsByCode Done');
}
