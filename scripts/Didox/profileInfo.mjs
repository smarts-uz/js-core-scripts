// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.profileInfo.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.profileInfo(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox profileInfo Start');
    ...
    const run = async () => {
        const result = await Didox.profileInfo();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox profileInfo Error');
    ...
    console.log('3️⃣ Didox profileInfo Done');
}
