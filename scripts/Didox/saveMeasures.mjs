// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.saveMeasures.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.saveMeasures(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox saveMeasures Start');
    ...
    const run = async () => {
        const result = await Didox.saveMeasures();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox saveMeasures Error');
    ...
    console.log('3️⃣ Didox saveMeasures Done');
}
