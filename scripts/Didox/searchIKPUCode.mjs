// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.searchIKPUCode.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.searchIKPUCode(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox searchIKPUCode Start');
    ...
    const run = async () => {
        const result = await Didox.searchIKPUCode(argv["text"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox searchIKPUCode Error');
    ...
    console.log('3️⃣ Didox searchIKPUCode Done');
}
