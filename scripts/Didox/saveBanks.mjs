// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.saveBanks.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.saveBanks(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox saveBanks Start');
    ...
    const run = async () => {
        const result = await Didox.saveBanks();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox saveBanks Error');
    ...
    console.log('3️⃣ Didox saveBanks Done');
}
