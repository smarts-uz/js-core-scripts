// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.getTaxpayerType.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.getTaxpayerType(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox getTaxpayerType Start');
    ...
    const run = async () => {
        const result = await Didox.getTaxpayerType();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox getTaxpayerType Error');
    ...
    console.log('3️⃣ Didox getTaxpayerType Done');
}
