// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.bankByCode.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.bankByCode(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox bankByCode Start');
    ...
    const run = async () => {
        const result = await Didox.bankByCode(argv["code"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox bankByCode Error');
    ...
    console.log('3️⃣ Didox bankByCode Done');
}
