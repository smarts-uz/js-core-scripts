// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.vatRegStatus.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.vatRegStatus(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox vatRegStatus Start');
    ...
    const run = async () => {
        const result = await Didox.vatRegStatus(argv["tin"], argv["date"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox vatRegStatus Error');
    ...
    console.log('3️⃣ Didox vatRegStatus Done');
}
