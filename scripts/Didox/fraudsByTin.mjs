// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.fraudsByTin.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.fraudsByTin(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox fraudsByTin Start');
    ...
    const run = async () => {
        const result = await Didox.fraudsByTin(argv["tin"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox fraudsByTin Error');
    ...
    console.log('3️⃣ Didox fraudsByTin Done');
}
