// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Didox.carInfoByPinfl.
// Edit the method in classes/Didox.js, then re-run the generator to refresh.
// Calls Didox.carInfoByPinfl(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Didox carInfoByPinfl Start');
    ...
    const run = async () => {
        const result = await Didox.carInfoByPinfl(argv["tin"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Didox carInfoByPinfl Error');
    ...
    console.log('3️⃣ Didox carInfoByPinfl Done');
}
