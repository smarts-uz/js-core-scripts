// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for PowerPoints.protectFileAsk.
// Edit the method in classes/PowerPoints.js, then re-run the generator to refresh.
// Calls PowerPoints.protectFileAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ PowerPoints protectFileAsk Start');
    ...
    const run = async () => {
        const result = await PowerPoints.protectFileAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'PowerPoints protectFileAsk Error');
    ...
    console.log('3️⃣ PowerPoints protectFileAsk Done');
}
