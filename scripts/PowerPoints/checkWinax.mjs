// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for PowerPoints.checkWinax.
// Edit the method in classes/PowerPoints.js, then re-run the generator to refresh.
// Calls PowerPoints.checkWinax(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ PowerPoints checkWinax Start');

    const argv = yargs(hideBin(process.argv))
        .option("methodName", { describe: "methodName" })
        .help()
        .parse();

    const run = async () => {
        const result = await PowerPoints.checkWinax(argv["methodName"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'PowerPoints checkWinax Error');
    ...
    console.log('3️⃣ PowerPoints checkWinax Done');
}
