// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Scanner.isExcluded.
// Edit the method in classes/Scanner.js, then re-run the generator to refresh.
// Calls Scanner.isExcluded(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Scanner isExcluded Start');
    ...
    const run = async () => {
        const result = await Scanner.isExcluded(argv["name"], argv["exclusions"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Scanner isExcluded Error');
    ...
    console.log('3️⃣ Scanner isExcluded Done');
}
