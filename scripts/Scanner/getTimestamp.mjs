// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Scanner.getTimestamp.
// Edit the method in classes/Scanner.js, then re-run the generator to refresh.
// Calls Scanner.getTimestamp(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Scanner getTimestamp Start');
    ...
    const run = async () => {
        const result = await Scanner.getTimestamp();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Scanner getTimestamp Error');
    ...
    console.log('3️⃣ Scanner getTimestamp Done');
}
