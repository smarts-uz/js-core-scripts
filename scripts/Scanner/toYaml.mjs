// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Scanner.toYaml.
// Edit the method in classes/Scanner.js, then re-run the generator to refresh.
// Calls Scanner.toYaml(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Scanner toYaml Start');

    const argv = yargs(hideBin(process.argv))
        .option("obj", { describe: "obj" })
        .option("indent", { describe: "indent (optional)" })
        .help()
        .parse();

    const run = async () => {
        const result = await Scanner.toYaml(argv["obj"], argv["indent"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Scanner toYaml Error');
    ...
    console.log('3️⃣ Scanner toYaml Done');
}
