// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for MySoliq.companyInfoAPI.
// Edit the method in classes/MySoliq.js, then re-run the generator to refresh.
// Calls MySoliq.companyInfoAPI(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ MySoliq companyInfoAPI Start');
    ...
    const run = async () => {
        const result = await MySoliq.companyInfoAPI(argv["tin"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'MySoliq companyInfoAPI Error');
    ...
    console.log('3️⃣ MySoliq companyInfoAPI Done');
}
