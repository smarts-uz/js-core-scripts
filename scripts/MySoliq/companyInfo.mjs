// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for MySoliq.companyInfo.
// Edit the method in classes/MySoliq.js, then re-run the generator to refresh.
// Calls MySoliq.companyInfo(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ MySoliq companyInfo Start');
    ...
    const run = async () => {
        const result = await MySoliq.companyInfo(argv["tin"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'MySoliq companyInfo Error');
    ...
    console.log('3️⃣ MySoliq companyInfo Done');
}
