// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for MySoliq.vatInfoAPI.
// Edit the method in classes/MySoliq.js, then re-run the generator to refresh.
// Calls MySoliq.vatInfoAPI(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ MySoliq vatInfoAPI Start');
    ...
    const run = async () => {
        const result = await MySoliq.vatInfoAPI(argv["tin"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'MySoliq vatInfoAPI Error');
    ...
    console.log('3️⃣ MySoliq vatInfoAPI Done');
}
