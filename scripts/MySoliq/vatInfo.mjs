// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for MySoliq.vatInfo.
// Edit the method in classes/MySoliq.js, then re-run the generator to refresh.
// Calls MySoliq.vatInfo(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ MySoliq vatInfo Start');
    ...
    const run = async () => {
        const result = await MySoliq.vatInfo(argv["tin"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'MySoliq vatInfo Error');
    ...
    console.log('3️⃣ MySoliq vatInfo Done');
}
