// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.saveUrlFile.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.saveUrlFile(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes saveUrlFile Start');
    ...
    const run = async () => {
        const result = await Chromes.saveUrlFile(argv["file"], argv["url"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes saveUrlFile Error');
    ...
    console.log('3️⃣ Chromes saveUrlFile Done');
}
