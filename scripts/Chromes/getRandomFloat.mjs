// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.getRandomFloat.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.getRandomFloat(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes getRandomFloat Start');

    const argv = yargs(hideBin(process.argv))
        .option("min", { describe: "min" })
        .option("max", { describe: "max" })
        .help()
        .parse();

    const run = async () => {
        const result = await Chromes.getRandomFloat(argv["min"], argv["max"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes getRandomFloat Error');
    ...
    console.log('3️⃣ Chromes getRandomFloat Done');
}
