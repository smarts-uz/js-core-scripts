// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.getUrlFromFile.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.getUrlFromFile(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes getUrlFromFile Start');
    ...
    const run = async () => {
        const result = await Chromes.getUrlFromFile(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes getUrlFromFile Error');
    ...
    console.log('3️⃣ Chromes getUrlFromFile Done');
}
