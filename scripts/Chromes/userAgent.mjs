// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.userAgent.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.userAgent(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes userAgent Start');
    ...
    const run = async () => {
        const result = await Chromes.userAgent();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes userAgent Error');
    ...
    console.log('3️⃣ Chromes userAgent Done');
}
