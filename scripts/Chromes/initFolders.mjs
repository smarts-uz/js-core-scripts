// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.initFolders.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.initFolders(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes initFolders Start');

    const argv = yargs(hideBin(process.argv))
        .option("app", { describe: "app" })
        .help()
        .parse();

    const run = async () => {
        const result = await Chromes.initFolders(argv["app"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes initFolders Error');
    ...
    console.log('3️⃣ Chromes initFolders Done');
}
