// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.pageSetup.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.pageSetup(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes pageSetup Start');
    ...
    const run = async () => {
        const result = await Chromes.pageSetup();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes pageSetup Error');
    ...
    console.log('3️⃣ Chromes pageSetup Done');
}
