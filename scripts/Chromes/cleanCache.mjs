// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.cleanCache.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.cleanCache(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes cleanCache Start');
    ...
    const run = async () => {
        const result = await Chromes.cleanCache();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes cleanCache Error');
    ...
    console.log('3️⃣ Chromes cleanCache Done');
}
