// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.finish.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.finish(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes finish Start');
    ...
    const run = async () => {
        const result = await Chromes.finish();
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes finish Error');
    ...
    console.log('3️⃣ Chromes finish Done');
}
