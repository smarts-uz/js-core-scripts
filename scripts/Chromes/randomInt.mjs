// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Chromes.randomInt.
// Edit the method in classes/Chromes.js, then re-run the generator to refresh.
// Calls Chromes.randomInt(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Chromes randomInt Start');
    ...
    const run = async () => {
        const result = await Chromes.randomInt(argv["min"], argv["max"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Chromes randomInt Error');
    ...
    console.log('3️⃣ Chromes randomInt Done');
}
