// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.phoneToFolder.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.phoneToFolder(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone phoneToFolder Start');

    const argv = yargs(hideBin(process.argv))
        .option("phones", { describe: "phones" })
        .help()
        .parse();

    const run = async () => {
        const result = await Phone.phoneToFolder(argv["phones"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone phoneToFolder Error');
    ...
    console.log('3️⃣ Phone phoneToFolder Done');
}
