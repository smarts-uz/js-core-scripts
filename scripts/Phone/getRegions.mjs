// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.getRegions.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.getRegions(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone getRegions Start');
    ...
    const run = async () => {
        const result = await Phone.getRegions(argv["userDir"], argv["fullPath"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone getRegions Error');
    ...
    console.log('3️⃣ Phone getRegions Done');
}
