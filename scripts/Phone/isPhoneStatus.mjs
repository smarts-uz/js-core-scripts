// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.isPhoneStatus.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.isPhoneStatus(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone isPhoneStatus Start');

    const argv = yargs(hideBin(process.argv))
        .option("file", { describe: "file" })
        .help()
        .parse();

    const run = async () => {
        const result = await Phone.isPhoneStatus(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone isPhoneStatus Error');
    ...
    console.log('3️⃣ Phone isPhoneStatus Done');
}
