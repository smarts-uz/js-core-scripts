// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.phoneToFolderItem.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.phoneToFolderItem(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone phoneToFolderItem Start');

    const argv = yargs(hideBin(process.argv))
        .option("phone", { describe: "phone" })
        .help()
        .parse();

    const run = async () => {
        const result = await Phone.phoneToFolderItem(argv["phone"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone phoneToFolderItem Error');
    ...
    console.log('3️⃣ Phone phoneToFolderItem Done');
}
