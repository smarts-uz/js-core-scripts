// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.isRegion.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.isRegion(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone isRegion Start');
    ...
    const run = async () => {
        const result = await Phone.isRegion(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone isRegion Error');
    ...
    console.log('3️⃣ Phone isRegion Done');
}
