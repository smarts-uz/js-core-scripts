// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.isPhone.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.isPhone(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone isPhone Start');
    ...
    const run = async () => {
        const result = await Phone.isPhone(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone isPhone Error');
    ...
    console.log('3️⃣ Phone isPhone Done');
}
