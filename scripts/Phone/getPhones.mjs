// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Phone.getPhones.
// Edit the method in classes/Phone.js, then re-run the generator to refresh.
// Calls Phone.getPhones(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Phone getPhones Start');
    ...
    const run = async () => {
        const result = await Phone.getPhones(argv["userDir"], argv["fullPath"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Phone getPhones Error');
    ...
    console.log('3️⃣ Phone getPhones Done');
}
