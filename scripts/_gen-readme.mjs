...
//
//   # <Class>          (one heading per classes/ class with public static methods)
...
// its reflected signature, the scripts/<Class>/<method>.mjs run command, and a
// per-parameter table. Re-run whenever the classes/ API changes:
//   node scripts/_gen-readme.mjs
...
function runCommand(className, methodName, params) {
    const rel = `scripts/${className}/${methodName}.mjs`;
...
async function main() {
    ...
    for (const file of classFiles.sort()) {
        ...
        const lines = [
            `# ${className}`,
            '',
            `Runners: \`scripts/${className}/\` — ${methods.length} public static method(s).`,
            '',
        ];
    ...
    const header = [
        ...
        '',
        '> **Auto-generated** by [`scripts/_gen-readme.mjs`](scripts/_gen-readme.mjs).',
        '> Do not edit by hand — change the JSDoc in `classes/<Class>.js` and re-run',
        '> `node scripts/_gen-readme.mjs`.',
        '',
        'Every public static method of every class under [`classes/`](classes/) has a',
        'dedicated runner at `scripts/<Class>/<method>.mjs`, generated by',
        '[`scripts/_generate.mjs`](scripts/_generate.mjs). Each runner parses CLI args',
        '(alias flags like',
        ...
        '',
        '```bash',
        'node scripts/Homoglyph/word.mjs --file "report.docx" --chars "STy"',
        'node scripts/Markdown/merge.mjs one.md two.md',
        'node scripts/Scanner/run.mjs --sourceFolder "d:\\\\Statistic" --maxLevel 5',
        '```',
        '',
