// Generates STANDALONE per-method runners: scripts/<Class>/<method>.mjs for every
// public static (non "_"-prefixed) method of every class under classes/ (or a
...
//
//   node scripts/_generate.mjs                  # all classes
//   node scripts/_generate.mjs Homoglyph Word   # only the named classes
//
// scripts/_generate.mjs sits one level below the project root inside scripts/,
// so classes/ is ../classes and the generated runners (scripts/<Class>/x.mjs,
// two levels down) import the class as ../../classes/<Class>.js.
...
async function importClass(className) {
    // LIB_OVERRIDES paths are written relative to a generated runner's own location
    // (scripts/<Class>/<method>.mjs, two levels below ROOT — '../../'), so resolve
    // them from ROOT here, not from this generator script's own one-level-below-root folder.
    const filePath = LIB_OVERRIDES[className]
        ? path.resolve(ROOT, LIB_OVERRIDES[className].replace(/^\.\.\/\.\.\//, ''))
        : path.join(UTILS_DIR, `${className}.js`);
...
function runnerSource(className, methodName, reflected) {
    ...
    const helperImports = ['Yamls', 'Dates', 'Dialogs']
        ...
        .map((h) => `const { ${h} } = await import('${libImportPath(h)}');`)
        .join('\n');

    const libNote = LIB_OVERRIDES[className]
        ? `// Edit the method in ${LIB_OVERRIDES[className].replace(/^\.\.\/\.\.\//, '')}, then re-run the generator to refresh.`
        : `// Edit the method in classes/${className}.js, then re-run the generator to refresh.`;

    return `// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for ${className}.${methodName}.
${libNote}
...
// config.yml) from dirname(process.argv[1]). scripts/${className}/ is two levels
// below root, so '..','..' = root.
import path from 'node:path';
...
async function generateClass(className) {
    ...
    console.log(`✅ scripts/${className}/ — ${methods.length} runner(s): ${methods.join(', ')}`);
    return methods.length;
}

// Pure helpers are exported for unit testing; the generation run below only
// fires when this file is the process entry (node scripts/_generate.mjs …), so
