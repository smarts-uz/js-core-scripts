// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.unProtectFileAsk.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.unProtectFileAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels unProtectFileAsk Start');
    ...
    const run = async () => {
        const result = await Excels.unProtectFileAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels unProtectFileAsk Error');
    ...
    console.log('3️⃣ Excels unProtectFileAsk Done');
}
