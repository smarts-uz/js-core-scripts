// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.processPriceMon.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.processPriceMon(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels processPriceMon Start');
    ...
    const run = async () => {
        const result = await Excels.processPriceMon(argv["yamlData"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels processPriceMon Error');
    ...
    console.log('3️⃣ Excels processPriceMon Done');
}
