// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.getProtectedPath.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.getProtectedPath(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels getProtectedPath Start');
    ...
    const run = async () => {
        const result = await Excels.getProtectedPath(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels getProtectedPath Error');
    ...
    console.log('3️⃣ Excels getProtectedPath Done');
}
