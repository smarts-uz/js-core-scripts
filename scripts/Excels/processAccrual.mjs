// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.processAccrual.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.processAccrual(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels processAccrual Start');
    ...
    const run = async () => {
        const result = await Excels.processAccrual(argv["yamlData"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels processAccrual Error');
    ...
    console.log('3️⃣ Excels processAccrual Done');
}
