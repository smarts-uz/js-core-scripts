// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.isHidden.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.isHidden(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels isHidden Start');

    const argv = yargs(hideBin(process.argv))
        .option("file", { alias: "f", describe: "filename" })
        .option("sheetName", { describe: "sheetName" })
        .help()
        .parse();

    const run = async () => {
        const result = await Excels.isHidden(argv["file"], argv["sheetName"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels isHidden Error');
    ...
    console.log('3️⃣ Excels isHidden Done');
}
