// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.protectFileAsk.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.protectFileAsk(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels protectFileAsk Start');
    ...
    const run = async () => {
        const result = await Excels.protectFileAsk(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels protectFileAsk Error');
    ...
    console.log('3️⃣ Excels protectFileAsk Done');
}
