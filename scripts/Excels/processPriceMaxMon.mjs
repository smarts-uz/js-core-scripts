// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.processPriceMaxMon.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.processPriceMaxMon(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels processPriceMaxMon Start');
    ...
    const run = async () => {
        const result = await Excels.processPriceMaxMon(argv["yamlData"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels processPriceMaxMon Error');
    ...
    console.log('3️⃣ Excels processPriceMaxMon Done');
}
