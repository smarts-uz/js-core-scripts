// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Excels.processPayment.
// Edit the method in classes/Excels.js, then re-run the generator to refresh.
// Calls Excels.processPayment(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Excels processPayment Start');
    ...
    const run = async () => {
        const result = await Excels.processPayment(argv["yamlData"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Excels processPayment Error');
    ...
    console.log('3️⃣ Excels processPayment Done');
}
