// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Com.killOrphans.
// Edit the method in classes/Com.js, then re-run the generator to refresh.
// Calls Com.killOrphans(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Com killOrphans Start');

    const argv = yargs(hideBin(process.argv))
        .option("imageName", { describe: "imageName" })
        .option("before", { describe: "before" })
        .help()
        .parse();

    const run = async () => {
        const result = await Com.killOrphans(argv["imageName"], argv["before"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Com killOrphans Error');
    ...
    console.log('3️⃣ Com killOrphans Done');
}
