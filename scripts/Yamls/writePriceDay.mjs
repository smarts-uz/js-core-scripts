// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.writePriceDay.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.writePriceDay(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls writePriceDay Start');
    ...
    const run = async () => {
        const result = await Yamls.writePriceDay(argv["file"], argv["priceDay"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls writePriceDay Error');
    ...
    console.log('3️⃣ Yamls writePriceDay Done');
}
