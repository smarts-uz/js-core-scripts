// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.buildAccountEntries.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.buildAccountEntries(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls buildAccountEntries Start');
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls buildAccountEntries Error');
    ...
    console.log('3️⃣ Yamls buildAccountEntries Done');
}
