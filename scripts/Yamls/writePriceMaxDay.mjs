// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.writePriceMaxDay.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.writePriceMaxDay(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls writePriceMaxDay Start');
    ...
    const run = async () => {
        const result = await Yamls.writePriceMaxDay(argv["file"], argv["priceMaxDay"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls writePriceMaxDay Error');
    ...
    console.log('3️⃣ Yamls writePriceMaxDay Done');
}
