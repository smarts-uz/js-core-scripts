// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.writePriceApp.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.writePriceApp(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls writePriceApp Start');
    ...
    const run = async () => {
        const result = await Yamls.writePriceApp(argv["file"], argv["priceApp"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls writePriceApp Error');
    ...
    console.log('3️⃣ Yamls writePriceApp Done');
}
