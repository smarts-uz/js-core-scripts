// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.writePriceMon.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.writePriceMon(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls writePriceMon Start');
    ...
    const run = async () => {
        const result = await Yamls.writePriceMon(argv["file"], argv["priceMon"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls writePriceMon Error');
    ...
    console.log('3️⃣ Yamls writePriceMon Done');
}
