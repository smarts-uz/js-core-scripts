// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.writePriceMaxApp.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.writePriceMaxApp(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls writePriceMaxApp Start');
    ...
    const run = async () => {
        const result = await Yamls.writePriceMaxApp(argv["file"], argv["priceMaxApp"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls writePriceMaxApp Error');
    ...
    console.log('3️⃣ Yamls writePriceMaxApp Done');
}
