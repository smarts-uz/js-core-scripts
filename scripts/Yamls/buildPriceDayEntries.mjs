// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.buildPriceDayEntries.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.buildPriceDayEntries(...) directly; no shared engine.
//
...
async function main() {
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls buildPriceDayEntries Error');
    ...
    console.log('3️⃣ Yamls buildPriceDayEntries Done');
}
