// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Yamls.writeHistory.
// Edit the method in classes/Yamls.js, then re-run the generator to refresh.
// Calls Yamls.writeHistory(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Yamls writeHistory Start');
    ...
    const run = async () => {
        const result = await Yamls.writeHistory(argv["file"], argv["history"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Yamls writeHistory Error');
    ...
    console.log('3️⃣ Yamls writeHistory Done');
}
