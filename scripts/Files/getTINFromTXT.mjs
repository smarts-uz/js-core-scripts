// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.getTINFromTXT.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.getTINFromTXT(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files getTINFromTXT Start');
    ...
    const run = async () => {
        const result = await Files.getTINFromTXT(argv["folderCompan"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files getTINFromTXT Error');
    ...
    console.log('3️⃣ Files getTINFromTXT Done');
}
