// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.writeJson.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.writeJson(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files writeJson Start');
    ...
    const run = async () => {
        const result = await Files.writeJson(argv["file"], argv["data"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files writeJson Error');
    ...
    console.log('3️⃣ Files writeJson Done');
}
