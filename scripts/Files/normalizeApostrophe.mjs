// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.normalizeApostrophe.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.normalizeApostrophe(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files normalizeApostrophe Start');
    ...
    const run = async () => {
        const result = await Files.normalizeApostrophe(argv["name"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files normalizeApostrophe Error');
    ...
    console.log('3️⃣ Files normalizeApostrophe Done');
}
