// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.isEmpty.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.isEmpty(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files isEmpty Start');
    ...
    const run = async () => {
        const result = await Files.isEmpty(argv["value"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files isEmpty Error');
    ...
    console.log('3️⃣ Files isEmpty Done');
}
