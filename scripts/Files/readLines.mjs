// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.readLines.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.readLines(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files readLines Start');
    ...
    const run = async () => {
        const result = await Files.readLines(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files readLines Error');
    ...
    console.log('3️⃣ Files readLines Done');
}
