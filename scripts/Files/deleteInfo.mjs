// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.deleteInfo.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.deleteInfo(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files deleteInfo Start');
    ...
    const run = async () => {
        const result = await Files.deleteInfo(argv["folder"], argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files deleteInfo Error');
    ...
    console.log('3️⃣ Files deleteInfo Done');
}
