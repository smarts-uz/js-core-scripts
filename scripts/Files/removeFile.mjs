// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.removeFile.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.removeFile(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files removeFile Start');
    ...
    const run = async () => {
        const result = await Files.removeFile(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files removeFile Error');
    ...
    console.log('3️⃣ Files removeFile Done');
}
