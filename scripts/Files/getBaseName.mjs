// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.getBaseName.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.getBaseName(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files getBaseName Start');

    const argv = yargs(hideBin(process.argv))
        .option("file", { alias: "f", describe: "filePath" })
        .option("ext", { describe: "ext" })
        .help()
        .parse();

    const run = async () => {
        const result = await Files.getBaseName(argv["file"], argv["ext"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files getBaseName Error');
    ...
    console.log('3️⃣ Files getBaseName Done');
}
