// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.readTextFile.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.readTextFile(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files readTextFile Start');
    ...
    const run = async () => {
        const result = await Files.readTextFile(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files readTextFile Error');
    ...
    console.log('3️⃣ Files readTextFile Done');
}
