// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.cleanPath.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.cleanPath(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files cleanPath Start');
    ...
    const run = async () => {
        const result = await Files.cleanPath(argv["p"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files cleanPath Error');
    ...
    console.log('3️⃣ Files cleanPath Done');
}
