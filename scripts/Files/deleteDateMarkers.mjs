// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.deleteDateMarkers.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.deleteDateMarkers(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files deleteDateMarkers Start');
    ...
    const run = async () => {
        const result = await Files.deleteDateMarkers(argv["folderCompan"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files deleteDateMarkers Error');
    ...
    console.log('3️⃣ Files deleteDateMarkers Done');
}
