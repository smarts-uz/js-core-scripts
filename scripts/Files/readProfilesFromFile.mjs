// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.readProfilesFromFile.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.readProfilesFromFile(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files readProfilesFromFile Start');
    ...
    const run = async () => {
        const result = await Files.readProfilesFromFile(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files readProfilesFromFile Error');
    ...
    console.log('3️⃣ Files readProfilesFromFile Done');
}
