// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.archiveFolder.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.archiveFolder(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files archiveFolder Start');
    ...
    const run = async () => {
        const result = await Files.archiveFolder(argv["folder"], argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files archiveFolder Error');
    ...
    console.log('3️⃣ Files archiveFolder Done');
}
