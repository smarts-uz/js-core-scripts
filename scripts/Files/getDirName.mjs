// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.getDirName.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.getDirName(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files getDirName Start');
    ...
    const run = async () => {
        const result = await Files.getDirName(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files getDirName Error');
    ...
    console.log('3️⃣ Files getDirName Done');
}
