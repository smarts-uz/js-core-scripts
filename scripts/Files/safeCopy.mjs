// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.safeCopy.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.safeCopy(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files safeCopy Start');
    ...
    const run = async () => {
        const result = await Files.safeCopy(argv["src"], argv["dest"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files safeCopy Error');
    ...
    console.log('3️⃣ Files safeCopy Done');
}
