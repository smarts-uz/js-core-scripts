// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.readJson.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.readJson(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files readJson Start');
    ...
    const run = async () => {
        const result = await Files.readJson(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files readJson Error');
    ...
    console.log('3️⃣ Files readJson Done');
}
