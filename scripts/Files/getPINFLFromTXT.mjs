// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.getPINFLFromTXT.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.getPINFLFromTXT(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files getPINFLFromTXT Start');
    ...
    const run = async () => {
        const result = await Files.getPINFLFromTXT(argv["folderCompan"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files getPINFLFromTXT Error');
    ...
    console.log('3️⃣ Files getPINFLFromTXT Done');
}
