// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.openFile.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.openFile(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files openFile Start');
    ...
    const run = async () => {
        const result = await Files.openFile(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files openFile Error');
    ...
    console.log('3️⃣ Files openFile Done');
}
