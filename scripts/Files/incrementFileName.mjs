// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.incrementFileName.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.incrementFileName(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files incrementFileName Start');
    ...
    const run = async () => {
        const result = await Files.incrementFileName(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files incrementFileName Error');
    ...
    console.log('3️⃣ Files incrementFileName Done');
}
