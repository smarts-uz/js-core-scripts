// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.mkdirIfNotExists.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.mkdirIfNotExists(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files mkdirIfNotExists Start');
    ...
    const run = async () => {
        const result = await Files.mkdirIfNotExists(argv["dirPath"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files mkdirIfNotExists Error');
    ...
    console.log('3️⃣ Files mkdirIfNotExists Done');
}
