// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.findAllContractFiles.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.findAllContractFiles(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files findAllContractFiles Start');
    ...
    const run = async () => {
        const result = await Files.findAllContractFiles(argv["dir"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files findAllContractFiles Error');
    ...
    console.log('3️⃣ Files findAllContractFiles Done');
}
