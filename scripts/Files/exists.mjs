// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.exists.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.exists(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files exists Start');
    ...
    const run = async () => {
        const result = await Files.exists(argv["file"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files exists Error');
    ...
    console.log('3️⃣ Files exists Done');
}
