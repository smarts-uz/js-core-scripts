// AUTO-GENERATED by scripts/_generate.mjs — standalone runner for Files.findRecursive.
// Edit the method in classes/Files.js, then re-run the generator to refresh.
// Calls Files.findRecursive(...) directly; no shared engine.
//
...
async function main() {
    console.log('1️⃣ Files findRecursive Start');
    ...
    const run = async () => {
        const result = await Files.findRecursive(argv["dir"], argv["condition"]);
    ...
    if (Yamls.getConfig('CmdLine.TryCatch') === 'true') {
        try {
            ...
            Dialogs.warningBox(error.message, 'Files findRecursive Error');
    ...
    console.log('3️⃣ Files findRecursive Done');
}
